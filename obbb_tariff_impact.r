...
raw <- read_excel("tbl_aug.xlsx", sheet = "F2", range = "A5:E15") %>% 
...
ggplot(bars, aes(x = decile, y = impact, fill = component)) +
  ...
  geom_text(data = dat, aes(x = decile, y = net_total,
            ...
            vjust = 0.5, hjust = -0.25, size = 4.0, color = "#b30059",fontface = "bold", inherit.aes = FALSE) +
  annotate(geom = "label",
           x = 7.00,
           y = 13000,
  ...
  theme(
    ...
    plot.title = element_text(size = 18, face = "bold", margin = margin(t = 10, b = 10)),
