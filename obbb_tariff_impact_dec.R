...
setwd("/Users/takayukitamura/Documents/R_Computing/obbb_tariff_impact")
rm(list = ls())
library(tidyverse)
library(showtext)
library(ggtext)
...
raw <- read_excel("tbl_dec_2025.xlsx", sheet = "F2", range = "A5:E15") %>% 
...
ggplot(bars, aes(x = decile, y = impact, fill = component)) +
  ...
  geom_text(data = dat, aes(x = decile, y = net_total,
            ...
            vjust = 0.5, hjust = -0.25, size = 4, color = "#b30059",fontface = "bold", inherit.aes = FALSE) +
  annotate(geom = "label",
           x = 7.00,
           y = 13000,
           label = "Top 30% of US households,\na household income of $171,790 or more, \ncould get benefits",
           colour = "#b30059",
           size = 4) +
  annotate(geom = "curve",
           x = 10, xend = 8.5,
  ...
  coord_cartesian(clip = "off", expand = TRUE,
                  xlim = c(1, 10.5)) +
  labs(
    title = "Impacts of OBBB and 2025 Tariffs by Income Decile (2025 USD) as of 11/17/2025",
  ...
  theme(
    ...
    plot.title = element_text(size = 16, face = "bold", margin = margin(t = 10, b = 10)),
